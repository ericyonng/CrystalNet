...
#   include <cstdarg>
	...
	#define TIXML_SSCANF	sscanf
		...
		{
			int len = 512;
            auto pool = KERNEL_NS::KernelGetTlsMemoryPool();

			for (;;) {
				len = len*2;
                char *str = reinterpret_cast<char *>(pool->AllocThreadLocal(static_cast<UInt64>(len)));
				const int required = _vsnprintf(str, len, format, va);
                pool->FreeThreadLocal(str);
				if ( required != -1 ) {
...
void StrPair::Reset()
{
    if ( _flags & NEEDS_DELETE ) {
        KERNEL_NS::KernelFreeMemory<KERNEL_NS::_Build::TL>(_start);
    }
    _flags = 0;
    _start = 0;
    _end = 0;
...
void StrPair::SetStr( const char* str, int flags )
{
    ...
    _start = reinterpret_cast<char *>(KERNEL_NS::KernelAllocMemory<KERNEL_NS::_Build::TL>(static_cast<UInt64>(len + 1)));
...
/*static*/ void XMLNode::DeleteNode( XMLNode* node )
{
	...
	if (!node->ToDocument()) {
		node->_document->Giveup(node);
	}

    node->~XMLNode();
    KERNEL_NS::KernelFreeMemory<KERNEL_NS::_Build::TL>(node);
}
...
void XMLNode::InsertChildPreamble( XMLNode* insertThis ) const
{
	...
	if (insertThis->_parent) {
		...
		insertThis->_document->Giveup(insertThis);
	}
}
...
{
    auto buf = reinterpret_cast<Byte8 *>(KERNEL_NS::KernelAllocMemory<KERNEL_NS::_Build::TL>(BUF_SIZE));
    XMLUtil::ToStr( v, buf, BUF_SIZE );
    _value.SetStr( buf );
    KERNEL_NS::KernelFreeMemory<KERNEL_NS::_Build::TL>(buf);
}
...
void XMLElement::DeleteAttribute( XMLAttribute* attribute )
{
    ...
    KERNEL_NS::KernelFreeMemory<KERNEL_NS::_Build::TL>(attribute);
}

XMLAttribute* XMLElement::CreateAttribute()
{
    auto mem = KERNEL_NS::KernelAllocMemory<KERNEL_NS::_Build::TL>(static_cast<UInt64>(sizeof(XMLAttribute)));
    XMLAttribute* attrib = KERNEL_NS::AllocUtil::NewByPtr<XMLAttribute>(mem);
    TIXMLASSERT( attrib );
    return attrib;
}
...
void XMLDocument::Giveup(const XMLNode* const node)
{
	TIXMLASSERT(node);
	TIXMLASSERT(node->_parent == 0);
...
{
    XMLElement* ele = CreateUnlinkedNode<XMLElement>();
    ele->SetName( name );
    return ele;
}
...
{
    XMLComment* comment = CreateUnlinkedNode<XMLComment>();
...
{
    XMLDeclaration* dec = CreateUnlinkedNode<XMLDeclaration>();
...
{
    XMLUnknown* unk = CreateUnlinkedNode<XMLUnknown>();
    unk->SetValue( str );
    return unk;
}
