...
* 线程数量 目前总数12个线程，后期会将线程数优化到8线程
  * 日志: 必须，3个线程（业务逻辑日志线程, 系统日志线程, 网络日志线程）
  * 业务逻辑: 必须，1个
  * 网络：必须，在必要情况下可以将监听的预数据传输的合并成1个poller，每个poller2个线程，包括至少一个监听poller，一个数据传输poller，至少4个线程，可以优化到只有一个poller（监听和数据传输共用一个poller）
  * 主线程: 必须，1个（可与系统性能监控，内存监控合并一条线程）
  * 内存监控：必须，1个（用于收集内存信息日志（内存池和对象池））
  * 系统性能监控：看情况而定，1个，用于收集网络，业务逻辑的性能指标
...
* 安装cmake不低于v 3.25.0-rc4

* 安装gcc 8.3以上

* 安装jenkins（doc中安装步骤， ci等功能）
