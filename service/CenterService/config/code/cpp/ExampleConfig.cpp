...
// example:column_1_10:{json text}|...
    ...
    {// _testDict
      ...
      {
          g_Log->Error(LOGFMT_OBJ_TAG("%s, assign fail field name:_testDict, data part:%s, errInfo:%s  line data:%s, pos:%d, headerTailPos:%d, dataEndPos:%d"), KERNEL_NS::RttiUtil::GetByObj(this), dataPart.c_str(), errInfo.c_str(), lineData.c_str(), static_cast<Int32>(pos), static_cast<Int32>(headerTailPos), static_cast<Int32>(dataEndPos));
          return false;
      }
...
void ExampleConfig::Serialize(KERNEL_NS::LibString &lineData) const
{
    ...
    {// _testDict
        KERNEL_NS::LibString data;
        SERVICE_COMMON_NS::DataTypeHelper::ToString(_testDict, data);
    ...
    {// _achieve
        ...
        lineData.AppendFormat("column_10_%d:%s|", static_cast<Int32>(data.size()), data.c_str());
        ++countFieldNum;
    }// _achieve
...
{
    ExampleConfigMgr::DeleteByAdapter_ExampleConfigMgr(ExampleConfigMgrFactory::_buildType.V, this);
}
...
Int32 ExampleConfigMgr::Load()
{
    ...
    const auto wholePath = basePath + "/" + "ExampleConfig.data";
    ...
    for(auto config : _configs)
    {
        ...
        // key:Type 
        {
            ...
            if(iterConfigs == _typeRefConfigs.end())
                iterConfigs = _typeRefConfigs.insert(std::make_pair(config->_type, std::vector<ExampleConfig *>())).first;
...
Int64 ExampleConfigMgr::_ReadConfigData(FILE &fp, KERNEL_NS::LibString &configData, Int32 totalLine, Int32 curLine) const
{
    ...
    std::set<Int32> needFieldIds = {2, 3, 4, 5, 6, 7, 8, 9, 10};
    const Int32 fieldNum = 9;
    Int32 count = 0;
    while(true)
    {
