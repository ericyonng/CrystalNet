...
// file path:../../service/TestService/config/xlsx/Œƒ◊÷id.xlsx
// sheet name:ÊñáÂ≠óid|WordId

#include <pch.h>
#include "WordIdConfig.h"
...
bool WordIdConfig::Parse(const KERNEL_NS::LibString &lineData)
{
...
void WordIdConfig::Serialize(KERNEL_NS::LibString &lineData) const
{
    const Int32 fieldNum = 2;
    Int32 countFieldNum = 0;
...
void WordIdConfigMgr::Release()
{
    WordIdConfigMgr::DeleteByAdapter_WordIdConfigMgr(WordIdConfigMgrFactory::_buildType.V, this);
}

void WordIdConfigMgr::Clear()
{
    _Clear();
}

KERNEL_NS::LibString WordIdConfigMgr::ToString() const
{
    return GetObjName();
}

Int32 WordIdConfigMgr::Load()
{
    KERNEL_NS::SmartPtr<std::vector<WordIdConfig *>, KERNEL_NS::AutoDelMethods::CustomDelete> configs = new std::vector<WordIdConfig *>;
    configs.SetClosureDelegate([](void *p){
        std::vector<WordIdConfig *> *ptr = reinterpret_cast<std::vector<WordIdConfig *> *>(p);
        KERNEL_NS::ContainerUtil::DelContainer(*ptr, [](WordIdConfig *ptr){
            WordIdConfig::Delete_WordIdConfig(ptr);
        });
        delete ptr;
    });

    const auto basePath = GetLoader()->GetBasePath();
    const auto wholePath = basePath + "/" + "WordIdConfig.data";
    ...
    while(true)
    {
        ...
        KERNEL_NS::SmartPtr<WordIdConfig, KERNEL_NS::AutoDelMethods::CustomDelete> config = WordIdConfig::New_WordIdConfig();
        config.SetClosureDelegate([](void *p){
            auto ptr = reinterpret_cast<WordIdConfig *>(p);
            WordIdConfig::Delete_WordIdConfig(ptr);
        });
...
Int32 WordIdConfigMgr::Reload()
{
    return Load();
}

const KERNEL_NS::LibString &WordIdConfigMgr::GetConfigDataMd5() const
{
    return _dataMd5;
}

void WordIdConfigMgr::_OnClose()
{
    _Clear();
}

void WordIdConfigMgr::_Clear()
{
    _dataMd5.clear();
    _idRefConfig.clear();


    KERNEL_NS::ContainerUtil::DelContainer(_configs, [](WordIdConfig *ptr){
        WordIdConfig::Delete_WordIdConfig(ptr);
    });
}

Int64 WordIdConfigMgr::_ReadConfigData(FILE &fp, KERNEL_NS::LibString &configData, Int32 totalLine, Int32 curLine) const
{
