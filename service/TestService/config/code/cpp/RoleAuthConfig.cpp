...
// example:column_1_10:{json text}|...
    ...
    {// _pwd
        ...
        if(pos == std::string::npos)
        {
            g_Log->Error(LOGFMT_OBJ_TAG("parse field:Pwd, data format error: have no column_ prefix, lineData:%s, startPos:%d, countFieldNum:%d"), lineData.c_str(), startPos, countFieldNum);
            return false;
        }
       ...
       if(headerTailPos == std::string::npos)
       {
            g_Log->Error(LOGFMT_OBJ_TAG("parse field:Pwd, bad line data not find : symbol after column_ line data:%s, startPos:%d, countFieldNum:%d"), lineData.c_str(), startPos, countFieldNum);
            return false;
       }
       ...
       if(headParts.empty())
       {
            g_Log->Error(LOGFMT_OBJ_TAG("parse field:Pwd, bad line data not find sep symbol:_ in header info line data:%s, pos:%d, headerTailPos:%d, startPos:%d, countFieldNum:%d,"),
            lineData.c_str(), static_cast<Int32>(pos), static_cast<Int32>(headerTailPos), startPos, countFieldNum);
            return false;
       }
       ...
       if(columnIdString.length() == 0)
       {
           g_Log->Error(LOGFMT_OBJ_TAG("parse field:Pwd have no column id, bad line data header len info line data:%s, pos:%d, headerTailPos:%d, startPos:%d, countFieldNum:%d,"), lineData.c_str(), static_cast<Int32>(pos), static_cast<Int32>(headerTailPos), startPos, countFieldNum);
           return false;
       }
       const UInt64 columnId = KERNEL_NS::StringUtil::StringToUInt64(columnIdString.c_str());
       if(columnId != 4)
       ...
       if(lenInfo.length() == 0)
       {
           g_Log->Error(LOGFMT_OBJ_TAG("parse field:Pwd fail: bad line data header len info line data:%s, pos:%d, headerTailPos:%d, startPos:%d, countFieldNum:%d"), lineData.c_str(), 
           static_cast<Int32>(pos), static_cast<Int32>(headerTailPos), startPos, countFieldNum);
           return false;
       }
      ...
      {
          g_Log->Error(LOGFMT_OBJ_TAG("%s, assign fail field name:_pwd, data part:%s, errInfo:%s  line data:%s, pos:%d, headerTailPos:%d, dataEndPos:%d"), KERNEL_NS::RttiUtil::GetByObj(this).c_str(), dataPart.c_str(), errInfo.c_str(), lineData.c_str(), static_cast<Int32>(pos), static_cast<Int32>(headerTailPos), static_cast<Int32>(dataEndPos));
          return false;
      }
...
POOL_CREATE_OBJ_DEFAULT_IMPL(RoleAuthConfigMgr);
...
Int32 RoleAuthConfigMgr::Load()
{
    ...
    while(true)
    {
        ...
        {
            g_Log->Warn(LOGFMT_OBJ_TAG("parse RoleAuthConfig fail data path:%s line:%d, lineData:%s"), wholePath.c_str(), line, lineData.c_str());
            return Status::Failed;
        }
