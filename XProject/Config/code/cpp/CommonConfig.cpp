...
// example:column_1_10:{json text}|...
    ...
    {// _value
        ...
        if(pos == std::string::npos)
        {
            CLOG_ERROR("parse field:Value, data format error: have no column_ prefix, lineData:%s, startPos:%d, countFieldNum:%d", lineData.c_str(), startPos, countFieldNum);
            return false;
        }
       ...
       if(headerTailPos == std::string::npos)
       {
            CLOG_ERROR("parse field:Value, bad line data not find : symbol after column_ line data:%s, startPos:%d, countFieldNum:%d", lineData.c_str(), startPos, countFieldNum);
            return false;
       }
       ...
       if(headParts.empty())
       {
            CLOG_ERROR("parse field:Value, bad line data not find sep symbol:_ in header info line data:%s, pos:%d, headerTailPos:%d, startPos:%d, countFieldNum:%d,",
            lineData.c_str(), static_cast<Int32>(pos), static_cast<Int32>(headerTailPos), startPos, countFieldNum);
            return false;
       }
       ...
       if(columnIdString.length() == 0)
       {
           CLOG_ERROR("parse field:Value have no column id, bad line data header len info line data:%s, pos:%d, headerTailPos:%d, startPos:%d, countFieldNum:%d,", lineData.c_str(), static_cast<Int32>(pos), static_cast<Int32>(headerTailPos), startPos, countFieldNum);
           return false;
       }
       const UInt64 columnId = KERNEL_NS::StringUtil::StringToUInt64(columnIdString.c_str());
       if(columnId != 3)
       {
           CLOG_ERROR("parse field:Value, fail: bad comumn id, columnId:%llu, real column id:3, please check if config data is old version, line data header len info line data:%s, pos:%d, headerTailPos:%d, startPos:%d, countFieldNum:%d", columnId, lineData.c_str(), static_cast<Int32>(pos), static_cast<Int32>(headerTailPos), startPos, countFieldNum);
           return false;
       }
       ...
       if(lenInfo.length() == 0)
       {
           CLOG_ERROR("parse field:Value fail: bad line data header len info line data:%s, pos:%d, headerTailPos:%d, startPos:%d, countFieldNum:%d", lineData.c_str(), 
           static_cast<Int32>(pos), static_cast<Int32>(headerTailPos), startPos, countFieldNum);
           return false;
       }
      ...
      {
          CLOG_ERROR("%s, assign fail field name:_value, data part:%s, errInfo:%s  line data:%s, pos:%d, headerTailPos:%d, dataEndPos:%d", KERNEL_NS::RttiUtil::GetByObj(this).c_str(), dataPart.c_str(), errInfo.c_str(), lineData.c_str(), static_cast<Int32>(pos), static_cast<Int32>(headerTailPos), static_cast<Int32>(dataEndPos));
          return false;
      }
    ...
    {// _int64Value
        ...
        if(pos == std::string::npos)
        {
            CLOG_ERROR("parse field:Int64Value, data format error: have no column_ prefix, lineData:%s, startPos:%d, countFieldNum:%d", lineData.c_str(), startPos, countFieldNum);
            return false;
        }
       ...
       if(headerTailPos == std::string::npos)
       {
            CLOG_ERROR("parse field:Int64Value, bad line data not find : symbol after column_ line data:%s, startPos:%d, countFieldNum:%d", lineData.c_str(), startPos, countFieldNum);
            return false;
       }
       ...
       if(headParts.empty())
       {
            CLOG_ERROR("parse field:Int64Value, bad line data not find sep symbol:_ in header info line data:%s, pos:%d, headerTailPos:%d, startPos:%d, countFieldNum:%d,",
            lineData.c_str(), static_cast<Int32>(pos), static_cast<Int32>(headerTailPos), startPos, countFieldNum);
            return false;
       }
       ...
       if(columnIdString.length() == 0)
       {
           CLOG_ERROR("parse field:Int64Value have no column id, bad line data header len info line data:%s, pos:%d, headerTailPos:%d, startPos:%d, countFieldNum:%d,", lineData.c_str(), static_cast<Int32>(pos), static_cast<Int32>(headerTailPos), startPos, countFieldNum);
           return false;
       }
       const UInt64 columnId = KERNEL_NS::StringUtil::StringToUInt64(columnIdString.c_str());
       if(columnId != 4)
       {
           CLOG_ERROR("parse field:Int64Value, fail: bad comumn id, columnId:%llu, real column id:4, please check if config data is old version, line data header len info line data:%s, pos:%d, headerTailPos:%d, startPos:%d, countFieldNum:%d", columnId, lineData.c_str(), static_cast<Int32>(pos), static_cast<Int32>(headerTailPos), startPos, countFieldNum);
           return false;
       }
       ...
       if(lenInfo.length() == 0)
       {
           CLOG_ERROR("parse field:Int64Value fail: bad line data header len info line data:%s, pos:%d, headerTailPos:%d, startPos:%d, countFieldNum:%d", lineData.c_str(), 
           static_cast<Int32>(pos), static_cast<Int32>(headerTailPos), startPos, countFieldNum);
           return false;
       }
      ...
      {
          CLOG_ERROR("%s, assign fail field name:_int64Value, data part:%s, errInfo:%s  line data:%s, pos:%d, headerTailPos:%d, dataEndPos:%d", KERNEL_NS::RttiUtil::GetByObj(this).c_str(), dataPart.c_str(), errInfo.c_str(), lineData.c_str(), static_cast<Int32>(pos), static_cast<Int32>(headerTailPos), static_cast<Int32>(dataEndPos));
          return false;
      }
    ...
    while(true)
    {
        ...
        if(line < 2)
        {
            ...
            if(line == 1)
            {
                ...
                {
                    CLOG_ERROR("current data not match this config data wholePath:%s, current data columns:%s, this config columns:Id|Value|Int64Value|StringValue|", wholePath.c_str(), lineData.c_str());
                    return Status::Failed;
                }
            }
            // total line data
        ...
        {
            CLOG_WARN("parse CommonConfig fail data path:%s line:%d, lineData:%s", wholePath.c_str(), line, lineData.c_str());
            return Status::Failed;
        }
