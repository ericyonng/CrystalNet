...
// file path:D:\workplace\Development\CrystalNet\CrystalNet\XProject\scripts\../Config/xlsx/Item道具.xlsx
// sheet name:道具|Item
...
bool ItemConfig::Parse(const KERNEL_NS::LibString &lineData)
{
...
void ItemConfig::Serialize(KERNEL_NS::LibString &lineData) const
{
    const Int32 fieldNum = 2;
    Int32 countFieldNum = 0;
...
void ItemConfigMgr::Release()
{
    ItemConfigMgr::DeleteByAdapter_ItemConfigMgr(ItemConfigMgrFactory::_buildType.V, this);
}

void ItemConfigMgr::Clear()
{
    _Clear();
}

KERNEL_NS::LibString ItemConfigMgr::ToString() const
{
    return GetObjName();
}

Int32 ItemConfigMgr::Load()
{
    KERNEL_NS::SmartPtr<std::vector<ItemConfig *>, KERNEL_NS::AutoDelMethods::CustomDelete> configs = new std::vector<ItemConfig *>;
    configs.SetClosureDelegate([](void *p){
        std::vector<ItemConfig *> *ptr = reinterpret_cast<std::vector<ItemConfig *> *>(p);
        KERNEL_NS::ContainerUtil::DelContainer(*ptr, [](ItemConfig *ptr){
            ItemConfig::Delete_ItemConfig(ptr);
        });
        delete ptr;
    });

    const auto basePath = GetLoader()->GetBasePath();
    const auto wholePath = basePath + "/" + "ItemConfig.data";
    ...
    while(true)
    {
        ...
        if(line < 2)
        {
            ...
            if(line == 1)
            {
                if(lineData != "Id|Type|")
                {
                    g_Log->Error(LOGFMT_OBJ_TAG("current data not match this config data wholePath:%s, current data columns:%s, this config columns:Id|Type|"), wholePath.c_str(), lineData.c_str());
                    return Status::Failed;
                }
            }
            // total line data
        ...
        KERNEL_NS::SmartPtr<ItemConfig, KERNEL_NS::AutoDelMethods::CustomDelete> config = ItemConfig::New_ItemConfig();
        config.SetClosureDelegate([](void *p){
            auto ptr = reinterpret_cast<ItemConfig *>(p);
            ItemConfig::Delete_ItemConfig(ptr);
        });

        if(!config->Parse(lineData))
        {
            g_Log->Warn(LOGFMT_OBJ_TAG("parse ItemConfig fail data path:%s line:%d, lineData:%s"), wholePath.c_str(), line, lineData.c_str());
            return Status::Failed;
        }
    ...
    for(auto config : _configs)
    {
        ...
        // key:Type 
        {
            ...
            if(iterConfigs == _typeRefConfigs.end())
                iterConfigs = _typeRefConfigs.insert(std::make_pair(config->_type, std::vector<ItemConfig *>())).first;
...
Int32 ItemConfigMgr::Reload()
{
    return Load();
}

const KERNEL_NS::LibString &ItemConfigMgr::GetConfigDataMd5() const
{
    return _dataMd5;
}

void ItemConfigMgr::_OnClose()
{
    _Clear();
}

void ItemConfigMgr::_Clear()
{
    _dataMd5.clear();
    _idRefConfig.clear();
    ...
    KERNEL_NS::ContainerUtil::DelContainer(_configs, [](ItemConfig *ptr){
        ItemConfig::Delete_ItemConfig(ptr);
    });
}

Int64 ItemConfigMgr::_ReadConfigData(FILE &fp, KERNEL_NS::LibString &configData, Int32 totalLine, Int32 curLine) const
{
