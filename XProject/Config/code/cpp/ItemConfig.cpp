...
// file path:C:\workplace\mine\CrystalNet\CrystalNet\CrystalNet\XProject\scripts\../Config/xlsx/Item道具.xlsx
// sheet name:道具|Item
    ...
    while(true)
    {
        ...
        if(line < 2)
        {
            ...
            if(line == 1)
            {
                if(lineData != "Id|Type|")
                {
                    CLOG_ERROR("current data not match this config data wholePath:%s, current data columns:%s, this config columns:Id|Type|", wholePath.c_str(), lineData.c_str());
                    return Status::Failed;
                }
            }
            // total line data
        ...
        {
            CLOG_WARN("parse ItemConfig fail data path:%s line:%d, lineData:%s", wholePath.c_str(), line, lineData.c_str());
            return Status::Failed;
        }
