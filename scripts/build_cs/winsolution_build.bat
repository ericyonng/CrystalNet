...
cd %ROOT_PATH%tools\premake && win_premake5.exe --file=premake5_cs.lua %choose%
...
if errorlevel 1 (
    ...
    echo Solution file path: build_cs/%choose%/CrystalNet_%choose%.sln
	Rem cancel support c++17
	Rem win_premake5.exe --file=fixcpp17_forwin.lua %choose%
   if "%1"=="" (
        explorer ..\..\build_cs\%choose%
    )
    if "%1"=="1" (
        explorer ..\..\build_cs\%choose%
    )
