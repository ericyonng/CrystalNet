#!/usr/bin/env bash
# @author EricYonng<120453674@qq.com>

# 创建conf文件
# 目标db目录
LOCAL_TARGET_DB_PATH=$1
# 端口号
LOCAL_TARGET_PORT=$2
# 复制集名
LOCAL_REPL_SET_NAME=$3
# 配置文件名
LOCAL_MONGOD_CONF_FILE_NAME=$4
# mongodb缓存大小(GB)
LOCAL_MONGOD_CACHE_SIZE=$5
# 缺省默认1GB
if [ -z "${LOCAL_MONGOD_CACHE_SIZE}" ]; then
    LOCAL_MONGOD_CACHE_SIZE=1
fi

if [ -z "${LOCAL_TARGET_PORT}" ]; then
    echo "please specify a port!!!"
    exit 1
fi

if [ -z "${LOCAL_REPL_SET_NAME}" ]; then
    echo "please specify a LOCAL_REPL_SET_NAME!!!"
    exit 1
fi

# 创建目录
if [ -z "${LOCAL_TARGET_DB_PATH}" ]; then
    echo "please specify a target db path!!!"
    exit 1
fi

# 配置名
if [ -z "${LOCAL_MONGOD_CONF_FILE_NAME}" ]; then
    echo "mongod conf name is empty!!!"
    exit 1
fi

MONGOD_CONF=${LOCAL_TARGET_DB_PATH}/${LOCAL_MONGOD_CONF_FILE_NAME}

echo 'systemLog:' > ${MONGOD_CONF}
echo '    destination: file' >> ${MONGOD_CONF}
echo "    path: ${LOCAL_TARGET_DB_PATH}/mongod.log" >> ${MONGOD_CONF}
echo "    logAppend: true" >> ${MONGOD_CONF}
echo -e "\n" >> ${MONGOD_CONF}
echo -e "storage:" >> ${MONGOD_CONF}
echo "    dbPath: ${LOCAL_TARGET_DB_PATH}" >> ${MONGOD_CONF}
echo "    wiredTiger:" >> ${MONGOD_CONF}
echo "        engineConfig:" >> ${MONGOD_CONF}
echo "            cacheSizeGB: ${LOCAL_MONGOD_CACHE_SIZE}" >> ${MONGOD_CONF}
echo -e "\n" >> ${MONGOD_CONF}
echo "net:" >> ${MONGOD_CONF}
echo "    bindIp: 0.0.0.0" >> ${MONGOD_CONF}
echo "    port: ${LOCAL_TARGET_PORT}" >> ${MONGOD_CONF}
echo -e "\n" >> ${MONGOD_CONF}
echo "replication:" >> ${MONGOD_CONF}
echo "    replSetName: ${LOCAL_REPL_SET_NAME}" >> ${MONGOD_CONF}
echo "    enableMajorityReadConcern: true" >> ${MONGOD_CONF}
echo -e "\n" >> ${MONGOD_CONF}
echo "processManagement:" >> ${MONGOD_CONF}
echo "    fork: true" >> ${MONGOD_CONF}

echo "mongod conf:${MONGOD_CONF} create success..."
