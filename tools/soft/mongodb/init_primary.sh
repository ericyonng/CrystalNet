...
# 先启动并创建用户, 密码与授权
sh ${SCRIPT_PATH}/create_mongodb_inst.sh ${SCRIPT_PATH} ${LOCAL_REPLISET_INSTALL_PATH}/${LOCAL_DB_NAME} ${LOCAL_PRIMARY_IP} ${LOCAL_PRIMARY_PORT} "${LOCAL_RS_NAME}" ${LOCAL_KEYFILE_PATH} "${LOCAL_SHARDING_CLUSTER_ROLE}" "${LOCAL_IS_MONGOS}" "${LOCAL_MONGOS_CONFIG_ADDR}" "no_auth" || {
    echo "创建主节点db1实例失败！" >&2
    exit 1
}
