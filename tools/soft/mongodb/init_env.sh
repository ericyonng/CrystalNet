...
# 安装
echo "执行 source ${TARGET_MACHINE_WORK_PATH}/install_mongodb.sh ${LOCAL_INSTALL_PATH}..."
source ${TARGET_MACHINE_WORK_PATH}/install_mongodb.sh ${LOCAL_INSTALL_PATH} || {
    echo "错误： 执行 source ${TARGET_MACHINE_WORK_PATH}/install_mongodb.sh 失败" >&2
    exit 1
}

echo "init env success, LOCAL_INSTALL_PATH:${LOCAL_INSTALL_PATH} script path:${TARGET_MACHINE_WORK_PATH} enjoy!!"
